...
import stcl.myNEAT.genotypes.JNeatGenomeIO;
...
public class JNeatGenomeIOTest {
	...
	@Test
	public void testReadGenomeFile() {
		//Find genome file
				String filepath = new File("").getAbsolutePath();
				filepath += "/Test documents/genomefile to create from";
				
				//Create genome it should be tested against
				createNodes();
				createConnections();
				Genome g = new Genome(0, inputNodes, outputNodes, hiddenNodes, connections);
				
				//Create genome
				Genome newG;
				JNeatGenomeIO genomeReader = new JNeatGenomeIO();
				try {
					newG = genomeReader.readGenomeFile(filepath);	
	...
	@Test
	public void testPrintGenomeToFile() {
		//Create nodes
		createNodes();
		...
		//Create genome file
				...
				JNeatGenomeIO genomeWriter = new JNeatGenomeIO();
				try {
					genomeWriter.printGenomeToFile(g, filepath);
				} catch (FileNotFoundException e) {
					e.printStackTrace();
					fail(e.getMessage());
				} catch (IOException e) {
